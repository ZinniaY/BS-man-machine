 ...
 */
 ...
 ;(function($,window,document){
 	...
 	function DialogBox(element,options){
 		this.element = element;//调用dialogBox函数的对象
 		this.settings = $.extend({}, defaults, options);//获取参数
 		this.init();//初始化  this指向当前对象		
 	}
 	...
 	DialogBox.prototype = {	
 		...
 		//初始化弹出框
 		init: function(){
 			var that = this,//调用函数的jquery对象,而不是window
 			...
 			if(!title){
 				...
 				if(!hasClose){
					dialogHTML[0] = '<section class="dialog-box"><div class="dialog-box-container"><div class="dialog-box-title"><h3>'+ title + '</h3></div><div class="dialog-box-content"></div>';
 					...
 					dialogHTML[0] = '<section class="dialog-box"><div class="dialog-box-container"><div class="dialog-box-title" style = "background-color:#13c8ee"><h3>'+ title + '</h3><span class="dialog-box-close">×</span></div><div class="dialog-box-content"></div>';
 				}
 			}
 		...
 		render: function(element){
 			var that = this,//jquery对象
 				$this = $(element),//将dom对象转化成jquery对象
 				dialogHTML = that.create($this),//对话框dom结构的字符串结构
 				$content = that.parseContent();//若内容是链接，则创建iframe,并渲染
 				
 			$this.replaceWith(dialogHTML[0] + dialogHTML[1]);//置换当前元素的内容，所有在svg应用中需要另外创建div
 			...
 			$('body').append(dialogHTML[2]);//在body中追加蒙版
 		},
 		...
 		parseContent: function(){
 			...
 			if(urlReg.test(content)){//如果内容为链接
